using MicroNet.Loan.Application;
using MicroNet.Loan.Core.Logging;
using MicroNet.Loan.Infrastructure;
using MicroNet.Loan.Infrastructure.Data;
using MicroNet.Loan.Infrastructure.Logging;
...
builder.Services.AddDbContext<LoanContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("LoanConnection")));
    ...
    options.AddSecurityDefinition("oauth2", new OpenApiSecurityScheme
    {
        ...
        Flows = new OpenApiOAuthFlows
        {
            AuthorizationCode = new OpenApiOAuthFlow
            {
                ...
                {
                    {"micronetloanapi", "MicroNet Loan Api - full access"}
                }
            },
        }
    });
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "MicroNet Loan Api");
...
else
{
    ...
    app.UseSwaggerUI(options =>
    {
        options.SwaggerEndpoint(builder.Configuration["AppSettings:Folder"] + "/swagger/v1/swagger.json", "MicroNet Loan Api");
        options.OAuthClientId(builder.Configuration["MicroNetIdpSettings:ApiName"]);
        options.OAuthRealm(" ");
        options.OAuthAppName(" ");
        options.OAuthUsePkce();
