...
{
    public class UpdateInterestDistributionCommandHandler : ICommandHandler<UpdateInterestDistributionCommand>
    {
        private readonly IInterestDistributionRepository _repository;
        //private readonly IAuditLogServiceClient _auditLogClient;

        public UpdateInterestDistributionCommandHandler(IInterestDistributionRepository repository) //, IAuditLogServiceClient auditLogClient)
        {
            _repository = repository;
            //_auditLogClient = auditLogClient;
        }

        public async Task<Unit> Handle(UpdateInterestDistributionCommand request, CancellationToken cancellationToken)
        {
            var existing = await _repository.GetByIdAsync(request.Id);

            if (existing == null)
                throw new InterestDistributionIdNotFoundException(request.Id);

            var dto = request.DistributionDto;
