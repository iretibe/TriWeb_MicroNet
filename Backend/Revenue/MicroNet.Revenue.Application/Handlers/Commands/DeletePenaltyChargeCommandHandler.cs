...
{
    internal class DeletePenaltyChargeCommandHandler : ICommandHandler<DeletePenaltyChargeCommand>
    {
        private readonly IPenaltyChargeRepository _repository;
        //private readonly IAuditLogServiceClient _auditLogClient;

        public DeletePenaltyChargeCommandHandler(IPenaltyChargeRepository repository)
        {
            _repository = repository;
            //_auditLogClient = auditLogClient;
        }

        public async Task<Unit> Handle(DeletePenaltyChargeCommand request, CancellationToken cancellationToken)
        {
            var result = await _repository.GetByIdAsync(request.Id);
            if (result == null) throw new PenaltyChargeIdNotFoundException(request.Id);
