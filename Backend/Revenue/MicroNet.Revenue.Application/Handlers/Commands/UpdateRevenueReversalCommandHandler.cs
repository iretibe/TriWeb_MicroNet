...
{
    public class UpdateRevenueReversalCommandHandler : ICommandHandler<UpdateRevenueReversalCommand>
    {
        private readonly IRevenueReversalRepository _repository;
        //private readonly IAuditLogServiceClient _auditLogClient;

        public UpdateRevenueReversalCommandHandler(IRevenueReversalRepository repository)//, IAuditLogServiceClient auditLogClient)
        {
            _repository = repository;
            //_auditLogClient = auditLogClient;
        }

        public async Task<Unit> Handle(UpdateRevenueReversalCommand request, CancellationToken cancellationToken)
        {
            var existing = await _repository.GetByIdAsync(request.Id);

            if (existing == null)
                throw new RevenueReversalIdNotFoundException(request.Id);
