using Consul;
using MicroNet.Employee.Application;
using MicroNet.Employee.Core.Clients;
using MicroNet.Employee.Infrastructure;
...
builder.Services.AddDbContext<EmployeeContext>(options =>
    options.UseSqlServer(builder.Configuration["ConnectionStrings:EmployeeConnection"]));
    ...
    options.AddSecurityDefinition("oauth2", new OpenApiSecurityScheme
    {
        ...
        Flows = new OpenApiOAuthFlows
        {
            AuthorizationCode = new OpenApiOAuthFlow
            {
                ...
                {
                    {"micronetemployeeapi", "MicroNet Employee Api - full access"}
                }
            },
        }
    });
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "MicroNet Employee Api");
...
else
{
    ...
    app.UseSwaggerUI(options =>
    {
        options.SwaggerEndpoint(builder.Configuration["AppSettings:Folder"] + "/swagger/v1/swagger.json", "MicroNet Employee Api");
        options.OAuthClientId(builder.Configuration["MicroNetIdpSettings:ApiName"]);
        options.OAuthRealm(" ");
        options.OAuthAppName(" ");
        options.OAuthUsePkce();
