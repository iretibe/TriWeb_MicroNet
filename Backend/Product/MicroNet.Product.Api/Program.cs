using Consul;
using MicroNet.Product.Application;
using MicroNet.Product.Core.Clients;
using MicroNet.Product.Infrastructure;
    ...
    options.AddSecurityDefinition("oauth2", new OpenApiSecurityScheme
    {
        ...
        Flows = new OpenApiOAuthFlows
        {
            AuthorizationCode = new OpenApiOAuthFlow
            {
                ...
                {
                    {"micronetproductapi", "MicroNet Product Api - full access"}
                }
            },
        }
    });
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "MicroNet Product Api");
...
else
{
    ...
    app.UseSwaggerUI(options =>
    {
        options.SwaggerEndpoint(builder.Configuration["AppSettings:Folder"] + "/swagger/v1/swagger.json", "MicroNet Product Api");
        options.OAuthClientId(builder.Configuration["MicroNetIdpSettings:ApiName"]);
        options.OAuthRealm(" ");
        options.OAuthAppName(" ");
        options.OAuthUsePkce();
